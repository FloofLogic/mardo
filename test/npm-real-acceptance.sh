...
[[ $version == "mardo 0.9.13" ]] || fail "local command reported $version"
[[ -x "$cache/versions/0.9.13/Mardo.app/Contents/Helpers/mardo" ]] ||
    fail "local command did not acquire its own verified app"
[[ ! -e "$cache/versions/0.9.13/Mardo.app/Contents/Helpers/md" ]] ||
...
[[ $npx_version == "mardo 0.9.13" ]] || fail "npx command reported $npx_version"
...
[[ $global_version == "mardo 0.9.13" ]] || fail "global command reported $global_version"
...
if [[ ${MARDO_NPM_GUI_ACCEPTANCE:-0} == 1 ]]; then
    test_app="$cache/versions/0.9.13/Mardo.app/Contents/MacOS/Mardo"
...
[[ -d "$cache/versions/0.9.13/Mardo.app" ]] || fail "npm uninstall silently deleted app cache"
...
[[ ! -d "$cache/versions/0.9.13" ]] || fail "explicit cache cleanup retained owned app"
