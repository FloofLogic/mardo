...
function safeRequest(url, label, deadlineMilliseconds = 300_000) {
  return new Promise((resolve, reject) => {
    let settled = false;
    let responseForDeadline = null;
    const deadline = setTimeout(() => {
      const error = new Error("absolute request deadline");
      if (responseForDeadline) responseForDeadline.destroy(error);
    ...
    request.on("error", () => {
      if (!responseForDeadline) clearDeadline();
...
async function fetchCanonicalRecord() {
  const response = await safeRequest(RECORD_URL, "canonical release record", 60_000);
  try {
...
function runningMardoCommands() {
  ...
  if (pids.length > 256 || pids.some((pid) => !/^[1-9][0-9]*$/.test(pid))) return null;
  const commands = [];
  for (const pid of pids) {
    const processResult = spawnSync("/bin/ps", ["-p", pid, "-o", "command="], {
      encoding: "utf8",
      maxBuffer: 32 * 1024,
      timeout: 5_000
    });
    if (!processResult.error && processResult.status === 1) continue;
    if (processResult.error || processResult.signal || processResult.status !== 0) return null;
    commands.push(processResult.stdout.trim());
  }
  return commands;
}

function cacheApplicationIsRunning(cache, options = {}) {
  const inspect = options.runningMardoCommands || runningMardoCommands;
  const commands = inspect();
  if (!Array.isArray(commands)) return true;
  const versionPattern = /^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$/;
  const prefix = `${cache.versions}${path.sep}`;
  return commands.some((command) => {
    if (typeof command !== "string" || !command.startsWith(prefix)) return false;
    const relative = command.slice(prefix.length);
    const version = relative.split(path.sep, 1)[0];
    if (!versionPattern.test(version)) return false;
    const executable = path.join(cache.versions, version, "Mardo.app", "Contents", "MacOS", "Mardo");
...
async function cleanCache(options = {}) {
  ...
  if (cacheApplicationIsRunning(cache, options)) {
...
function runNativeHelper(helper, arguments_) {
  const result = spawnSync(helper, arguments_, { stdio: "inherit", timeout: 45_000 });
